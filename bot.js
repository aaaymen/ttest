...
client.on("message", message => {
    if(message.content.startsWith(prefix + '1')) {
     let args = message.content.split(" ").slice(1);
      ...
      message.channel.send(`:ballot_box_with_check: TemporarySound : \`${nam}\``).then(c => setTimeout(() => c.edit(`<@${message.author.id}> :stopwatch:  انتهى وقت الروم الصوتي`), 3600000))  // 120000 دقيقتان
    }
    });
...
client.on('ready', () => {
var x = client.channels.get("541343465510797312");
if (x) x.join();
});
...
client.on('guildMemberAdd', member => {
  member.guild.fetchInvites().then(guildInvites => {
	...
	client.login('NTAyODEzMzQyNTQxNDE0NDM1.DzDW2Q.Go4_G6B51cmZNw7qM4XgjgwyOS8');
